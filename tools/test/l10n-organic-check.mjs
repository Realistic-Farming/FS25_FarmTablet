// l10n-organic-check.mjs - MAINTENANCE row 105's text bar for the Organic Management app (batch 8 of the
// remaining apps in the tablet translation wave; the pattern of l10n-farmadmin-personnel-check.mjs, #191).
//
// One app, on its own because it carries a design origin: the FarmTablet organic app build brief (v1.0,
// Arissani) and the organic traceability build brief both set "26 languages for all app strings" as an
// invariant, and this batch carries it (DESIGN-CHECK, CARRIES).
//   - Organic (OrganicApp.lua).
...
//
// Usage:  node tools/test/l10n-organic-check.mjs        Exit: 0 clean, 1 any failure.
...
const KEYS = [
  "ft_auto_certified", "ft_auto_conventional", "ft_auto_crop", "ft_auto_in_transition",
  "ft_auto_install_fs25_soilfertilizer_to_use_this_app", "ft_organic_already_opted_in",
  "ft_organic_barn_fmt", "ft_organic_batch_day_left_fmt", "ft_organic_batch_days_left_fmt",
  "ft_organic_batch_fmt", "ft_organic_batch_ready_fmt", "ft_organic_batch_safe", "ft_organic_batch_unsafe",
  "ft_organic_breaches_fmt", "ft_organic_cert_unavailable", "ft_organic_certification", "ft_organic_compost",
  "ft_organic_compost_na", "ft_organic_compost_none", "ft_organic_countdown_fmt", "ft_organic_elevated_risk",
  "ft_organic_feed_disease", "ft_organic_feed_share", "ft_organic_feed_tip", "ft_organic_field_fmt",
  "ft_organic_help_cert_body", "ft_organic_help_later_body", "ft_organic_help_later_title",
  "ft_organic_help_practices_body", "ft_organic_help_what_body", "ft_organic_herd_health",
  "ft_organic_livestock", "ft_organic_livestock_na", "ft_organic_market", "ft_organic_market_na",
  "ft_organic_mycotoxin", "ft_organic_no_barns", "ft_organic_no_fields", "ft_organic_no_fields_yet",
  "ft_organic_not_in_programme", "ft_organic_opt_in", "ft_organic_opt_in_unavailable", "ft_organic_opt_out",
  "ft_organic_practice_bonus_fmt", "ft_organic_practice_fatigue_fmt", "ft_organic_practice_needs_fert",
  "ft_organic_practice_no_data", "ft_organic_practice_om_fair", "ft_organic_practice_om_healthy",
  "ft_organic_practice_om_low", "ft_organic_practice_rotation_fmt", "ft_organic_practices",
  "ft_organic_sale_premium", "ft_organic_select_field", "ft_organic_selected_field_fmt",
  "ft_organic_storage_fraction", "ft_organic_subtitle", "ft_organic_trace_na", "ft_organic_traceability",
  "ft_ui_app_organic",
];
...
const OTHER_PR = {
  "ft_auto_field_2": "another bar's key (l10n-fieldjobs-check.mjs)",
  ...
  "ft_auto_view": "another bar's key (l10n-rotation-sysset-prostaff-check.mjs)",
  "ft_auto_what_this_is": "another bar's key (l10n-field-sentry-check.mjs)",
  "ft_rotation_no_soil": "another bar's key (l10n-rotation-sysset-prostaff-check.mjs)",
};
// Drawn literals with no key, each with its reason.
const NO_KEY = {
  "?": "the unknown-state mark, a symbol",
};
...
const SOURCES = [
  {"file": "src/apps/OrganicApp.lua", "keyFns": ["l10n", "l10nFormat"], "literals": true, "drawnTables": ["STATE_LABEL"]},
];
...
if (failures.length > 0) {
  ...
  console.log(`l10n-organic: ${failures.length} failure(s) over ${checked} entries (${KEYS.length} keys x ${locales.length} locales)`);
  process.exit(1);
}
console.log(`l10n-organic: PASS - ${checked} entries checked (${KEYS.length} keys x ${locales.length} locales), ${Object.keys(ALLOW).length} allowed identical pairs, ${Object.keys(ALLOW_ALL).length} own names; script rule: ${scriptChecked} values in ${locales.filter((l) => SCRIPT[l]).join(", ")}`);
