// l10n-roleplay-phone-check.mjs - MAINTENANCE row 105's text bar for the Invoices app (batch 11 of the
// remaining apps in the tablet translation wave; the pattern of l10n-updates-check.mjs, #196).
//
// One app: Invoices (RoleplayPhoneApp.lua), in both modes (FS25_RoleplayPhone's invoices, or the built-in
// ones): the help page, the header and summary, the invoice cards (party, description, amount, status,
// due line, PAY / CANCEL / DELETE), the empty state, + NEW, and the new-invoice form (party, description,
// amount steppers, due date, CANCEL / CREATE). A preset is stored in English and translated where it is
// drawn; the status words and the custom label sit in drawn tables.
...
//
// Usage:  node tools/test/l10n-roleplay-phone-check.mjs        Exit: 0 clean, 1 any failure.
...
const KEYS = [
  "ft_auto_14_days", "ft_auto_1k_2", "ft_auto_1k_3", "ft_auto_30_days", "ft_auto_60_days", "ft_auto_7_days",
  "ft_auto_90_days", "ft_auto_amount_2", "ft_auto_animal_care", "ft_auto_bank", "ft_auto_cancel",
  "ft_auto_consulting", "ft_auto_contractor", "ft_auto_create", "ft_auto_create_invoices_via_the_roleplayphone_app",
  "ft_auto_crop_sale", "ft_auto_custom", "ft_auto_custom_set_via_console", "ft_auto_delete", "ft_auto_description",
  "ft_auto_due_date", "ft_auto_due_today", "ft_auto_equipment_dealer", "ft_auto_equipment_rental",
  "ft_auto_farm_supply", "ft_auto_field_work", "ft_auto_grain_elevator", "ft_auto_incoming_money_owed_to_you",
  "ft_auto_invoice_tracker", "ft_auto_invoices", "ft_auto_land_lease", "ft_auto_loan_payment", "ft_auto_maintenance",
  "ft_auto_manage_via_roleplayphone_app", "ft_auto_municipality", "ft_auto_new", "ft_auto_new_invoice",
  "ft_auto_new_invoice_2", "ft_auto_no_due_date", "ft_auto_no_invoices_yet", "ft_auto_outgoing_money_you_owe",
  "ft_auto_overdue", "ft_auto_owed", "ft_auto_paid", "ft_auto_party", "ft_auto_pay", "ft_auto_pending_2",
  "ft_auto_receivable", "ft_auto_rejected", "ft_auto_roleplay_phone_integration", "ft_auto_roleplayphone",
  "ft_auto_service_fee", "ft_auto_set_an_amount_greater_than_0_to_create", "ft_auto_summary_bar", "ft_auto_supplier",
  "ft_auto_use_new_to_create_your_first_invoice", "ft_auto_vet_2", "ft_rpphone_due_date_fmt",
  "ft_rpphone_due_in_fmt", "ft_rpphone_due_in_one", "ft_rpphone_help_new_body", "ft_rpphone_help_phone_body",
  "ft_rpphone_help_summary_body", "ft_rpphone_help_tracker_body", "ft_rpphone_overdue_fmt", "ft_rpphone_overdue_one",
];

// Keys whose text is the same in every language, with the reason.
const ALLOW_ALL = {
  "ft_auto_roleplayphone": "RoleplayPhone, the phone mod's own name (FS25_RoleplayPhone), shown as the data source",
};
// (locale, key) pairs whose text legitimately equals English, each with its reason.
const ALLOW = {
  "da:ft_auto_bank": "Bank is the Danish word",
  "de:ft_auto_bank": "Bank is the German word",
  "hu:ft_auto_bank": "Bank is the Hungarian word",
  "id:ft_auto_bank": "Bank is the Indonesian word",
  "nl:ft_auto_bank": "Bank is the Dutch word",
  "no:ft_auto_bank": "Bank is the Norwegian word",
  "pl:ft_auto_bank": "Bank is the Polish word",
  "sv:ft_auto_bank": "Bank is the Swedish word",
};
...
const OTHER_PR = {
  "ft_auto_built_in_2": "another bar's key (l10n-appstore-check.mjs)",
  "ft_auto_transport": "another bar's key (l10n-animals-contracts-fleet-check.mjs)",
  "ft_auto_unknown": "another bar's key (l10n-animals-contracts-fleet-check.mjs)",
};
// Drawn literals with no key, each with its reason.
const NO_KEY = {
  "◄": "the cycler's left arrow, a symbol",
  "►": "the cycler's right arrow, a symbol",
};
...
const CONTAINS = [
  ["ft_rpphone_help_tracker_body", "ft_auto_pending_2"],
  ["ft_rpphone_help_tracker_body", "ft_auto_paid"],
  ["ft_rpphone_help_tracker_body", "ft_auto_overdue"],
  ["ft_rpphone_help_tracker_body", "ft_auto_rejected"],
  ["ft_rpphone_help_new_body", "ft_auto_new"],
  ["ft_auto_use_new_to_create_your_first_invoice", "ft_auto_new"],
];
...
const SOURCES = [
  {"file": "src/apps/RoleplayPhoneApp.lua", "keyFns": ["l10n", "l10nFormat"], "literals": true,
   "drawnTables": ["PARTY_PRESETS", "DESC_PRESETS", "STATUS_LABEL", "CUSTOM_SHOWN", "labels"], "drawnArgs": {"drawCycler": 3}},
];
...
if (failures.length > 0) {
  ...
  console.log(`l10n-roleplay-phone: ${failures.length} failure(s) over ${checked} entries (${KEYS.length} keys x ${locales.length} locales)`);
  process.exit(1);
}
console.log(`l10n-roleplay-phone: PASS - ${checked} entries checked (${KEYS.length} keys x ${locales.length} locales), ${Object.keys(ALLOW).length} allowed identical pairs, ${Object.keys(ALLOW_ALL).length} own names; script rule: ${scriptChecked} values in ${locales.filter((l) => SCRIPT[l]).join(", ")}`);
