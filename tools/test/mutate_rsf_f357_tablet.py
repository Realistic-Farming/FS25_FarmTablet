...
MUTATIONS = [
 ...
 ("T09-waiting-rows-get-a-number", APP,
  [("            if r.kind == \"LIVE\" and type(r.trust) == \"number\" then live[#live + 1] = r else others[#others + 1] = r end\n",
    "            r.trust = r.trust or 0\n            r.roleLabel = r.roleLabel or ((r.kind == \"PRESENCE\") and \"worker\" or \"waiting\")\n            live[#live + 1] = r\n", 1)],
...
# ── Bob's #168 verdict at db73cdb6 ──────────────────────────────────────────
 ("T15-note-appended-to-the-value", APP,
  [("    y = self:drawRow(y, \"Active Favors\", tostring(active))\n    if view.state == \"LAST_CONFIRMED\" then\n        y = self:drawRow(y, \"(last confirmed)\", \"\", FT.C.TEXT_DIM)\n    end\n",
    "    y = self:drawRow(y, \"Active Favors\", tostring(active) .. ((view.state == \"LAST_CONFIRMED\") and \"  (last confirmed)\" or \"\"))\n", 1)],
  "the note rides inside the count value, where no exact mapping can reach it"),
 ("T16-tag-composed-into-the-name", APP,
  [("                self.r:appText(x, y, FT.FONT.SMALL, nm, RenderText.ALIGN_LEFT, FT.C.TEXT_DIM)\n                self.r:appText(x + cw, y, FT.FONT.SMALL, tag, RenderText.ALIGN_RIGHT, FT.C.TEXT_DIM)\n",
    "                self.r:appText(x, y, FT.FONT.SMALL, nm .. \"  [\" .. tag .. \"]\", RenderText.ALIGN_LEFT, FT.C.TEXT_DIM)\n", 1)],
  "the tag is composed into the name, English in 25 languages"),
 ("T17-literals-unmapped", "src/core/Constants.lua",
  [("    [\"(last confirmed)\"] = \"ft_auto_last_confirmed\",\n", "", 1),
   ("    [\"time unknown\"] = \"ft_auto_time_unknown\",\n", "", 1),
   ("    [\"waiting\"] = \"ft_auto_waiting\",\n", "", 1)],
  "three new literals have no exact-text mapping and render English"),
 ("T18-disabled-tablet-keeps-watching", APP,
  ...
  "disabling the tablet with the NPC app watching never releases the watch"),
 ("T19-total-earned-figure-on-the-repaired-host", APP,
  [("    -- The host supplies no earnings summary: unavailable, never a raw figure.\n    y = self:drawRow(y, \"Total Earned\", \"unavailable\", nil, FT.C.TEXT_DIM)\n",
    "    local statsRaw = (npcSys.favorSystem and npcSys.favorSystem.stats) or {}\n    y = self:drawRow(y, \"Total Earned\", (g_i18n and g_i18n:formatMoney(statsRaw.totalMoneyEarned or 0, 0, true, true)) or \"$0\", nil, FT.C.POSITIVE)\n", 1)],
  "the raw earnings figure is drawn on the repaired host"),
]
