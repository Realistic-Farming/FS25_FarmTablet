...
// ---- ACCENT (batch 15's named defect, as batch 14's): each restored pair keeps its accents.
{
  ...
  for (const [loc, keys] of Object.entries(ACCENT)) {
    for (const key of keys) {
      n++;
      if (!KEYS.includes(key)) { failures.push(`ACCENT ${loc}: ${key} is not one of this bar's keys`); continue; }
      const v = unesc((ALL[loc].inside.get(key) || [""])[0]);
      if (fold(v) === v) failures.push(`ACCENT ${loc}: ${key} ${JSON.stringify(v)} carries no accent: the Akita83 import's stripped text is back`);
    }
  }
