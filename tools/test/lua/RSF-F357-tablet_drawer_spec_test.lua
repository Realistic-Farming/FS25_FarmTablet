--!load: src/core/Constants.lua, src/utils/Renderer.lua, src/FarmTabletUI.lua, src/apps/IncomeApp.lua
...
local function tablet()
    ...
    ui.r = { appText = function(_, x, y, font, text) ui.texts[#ui.texts + 1] = tostring(text) end, appRect = function() end }
    ...
    function ui:drawRow(y, label, value) self.rows[#self.rows + 1] = tostring(label) .. "=" .. tostring(value) return y - 10 end
...
group("H the work page", function()
    ...
    T.ok("H2 an unavailable page reads unavailable, never a zero, and the raw model is not read", has(ui.rows, "Work=unavailable") and not hasPrefix(ui.rows, "Active Favors=") and not has(ui.rows, "Completed=99") and not hasPrefix(ui.rows, "Total Earned="))
    draw(ui)
    ...
    T.ok("H5c an accepted row with no known time says so, never zero hours", has(ui.rows, "Anna  Watch property=20%  time unknown"))
    T.ok("H5d the raw model is never read on the repaired host", not has(ui.rows, "Completed=99") and not hasPrefix(ui.rows, "Total Earned="))
    h.view = { state = "LAST_CONFIRMED", totalKnown = true, total = 4, completedKnown = true, completedCount = 4, rows = CURRENT.rows }
    draw(ui)
    T.ok("H6 a last-confirmed page is labelled as such on its count", has(ui.rows, "Active Favors=2  (last confirmed)"))
...
group("N the roster view", function()
    ...
    local ui = draw(tablet())
    T.ok("N1 live people by trust with their role and a number", has(ui.sections, "RELATIONSHIPS  (2)") and has(ui.texts, "Anna  [farmer]") and has(ui.texts, "80  Friend") and has(ui.texts, "Ben  [?]") and has(ui.texts, "30  Cold"))
    ...
    T.ok("N2 a waiting person and an observed worker are named with no number", has(ui.texts, "Cara  [waiting]") and has(ui.texts, "Worker 1  [worker]") and not has(ui.texts, "0  Cold"))
