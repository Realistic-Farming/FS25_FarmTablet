// l10n-farmadmin-personnel-check.mjs - MAINTENANCE row 105's text bar for the Farm Admin and Personnel
// apps (batch 7 of the remaining apps in the tablet translation wave; the pattern of
// l10n-notes-excavator-check.mjs, #190).
//
// Two apps, kept near #187's size (Tyson's ruling for this pass):
//   - Farm Admin (FarmAdminApp.lua);
//   - Personnel (PersonnelApp.lua).
...
//
// Usage:  node tools/test/l10n-farmadmin-personnel-check.mjs        Exit: 0 clean, 1 any failure.
...
const KEYS = [
  ...
  "ft_auto_month_to_date", "ft_auto_net_savings", "ft_auto_net_surcharge", "ft_auto_no_candidates_available",
  "ft_auto_no_workers_yet_open_the_hire_tab_to_recruit", "ft_auto_payroll", "ft_auto_payroll_tab",
  "ft_auto_pin", "ft_auto_pro_staff_2", "ft_auto_pro_staff_impact", "ft_auto_recruitment_pool",
  "ft_auto_repair_all", "ft_auto_reroll", "ft_auto_roster", "ft_auto_roster_data_unavailable",
  "ft_auto_roster_tab", "ft_auto_running_costs", "ft_auto_skip_to", "ft_auto_sure",
  ...
  "ft_personnel_help_mp_body", "ft_personnel_help_payroll_body", "ft_personnel_help_roster_body",
  "ft_personnel_help_what_body", "ft_personnel_levels_fmt", "ft_personnel_msg_confirm_fire",
  "ft_personnel_msg_enter_vehicle", "ft_personnel_msg_fired", "ft_personnel_msg_hired",
  "ft_personnel_msg_no_vehicle_id", "ft_personnel_msg_pinned", "ft_personnel_msg_rerolled",
  "ft_personnel_msg_unpinned", "ft_personnel_per_worker_fmt", "ft_personnel_recruit_word",
  "ft_personnel_signing_cost_fmt", "ft_personnel_sort_fmt", "ft_personnel_vehicle_word",
  "ft_personnel_worker_stats_fmt", "ft_personnel_worker_word", "ft_personnel_workers_fmt",
  "ft_ui_app_farm_admin", "ft_ui_app_personnel",
];
...
const OTHER_PR = {
  "ft_auto_level": "another bar's key (l10n-rotation-sysset-prostaff-check.mjs)",
  "ft_auto_multiplayer": "another bar's key (l10n-field-sentry-check.mjs)",
  "ft_auto_name": "another bar's key (l10n-notes-excavator-check.mjs)",
  "ft_auto_status_2": "another bar's key (l10n-farmstats-production-events-market-check.mjs)",
  "ft_auto_unpin": "another bar's key (l10n-storage-usedplus-workshop-check.mjs)",
  "ft_auto_what_this_app_does": "another bar's key (l10n-rotation-sysset-prostaff-check.mjs)",
};
...
const NO_KEY = {
  ...
  "60×": "a time-scale multiplier, a number and the multiplication sign",
  "120×": "a time-scale multiplier, a number and the multiplication sign",
};
...
const SOURCES = [
  {"file": "src/apps/FarmAdminApp.lua", "keyFns": ["l10n", "l10nFormat"], "literals": true},
  {"file": "src/apps/PersonnelApp.lua", "keyFns": ["l10n", "l10nFormat"], "literals": true, "drawnTables": ["SORT_LABEL", "FILTER_LABEL"], "drawnArgs": {"addBtn": 5}},
];
...
if (failures.length > 0) {
  ...
  console.log(`l10n-farmadmin-personnel: ${failures.length} failure(s) over ${checked} entries (${KEYS.length} keys x ${locales.length} locales)`);
  process.exit(1);
}
console.log(`l10n-farmadmin-personnel: PASS - ${checked} entries checked (${KEYS.length} keys x ${locales.length} locales), ${Object.keys(ALLOW).length} allowed identical pairs, ${Object.keys(ALLOW_ALL).length} own names; script rule: ${scriptChecked} values in ${locales.filter((l) => SCRIPT[l]).join(", ")}`);
