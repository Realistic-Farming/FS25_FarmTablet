// l10n-animals-contracts-fleet-check.mjs - MAINTENANCE row 105's text bar for the Animal Husbandry, Contracts and
// Fleet Manager apps (batch 2 of the remaining apps in the tablet translation wave; the pattern of
// l10n-rotation-sysset-prostaff-check.mjs, #184).
//
// A batch of three apps (Tyson's ruling for this pass: 3 or 4 small apps per PR):
//   - Animal Husbandry (AnimalHusbandryApp.lua): the help page, the pen count, the no-pens and
//     purchase texts, and the food / water / straw bar labels;
//   - Contracts (ContractsApp.lua): the help page, the contract type names, the time-left texts, the
//     subtitle counts and summary badges, the status badge, the no-contracts and collect texts;
//   - Fleet Manager (FleetManagerApp.lua): the help page, the vehicle count, the no-vehicles texts,
//     the AI badge and the fuel / wear / hours labels.
// Drawn literals reach their keys through the renderer's FT.l10nAuto; built texts through FT.l10n,
// FT.l10nFormat and Fleet Manager's own ftFleetText. A SOURCES entry's drawnTables names a local table
// whose string values the app draws through FT.l10nAuto (the Contracts type names), and drawnArgs a
// local function whose argument at an index it draws that way (drawCard's status label, called through
// pcall): each such value is checked as a drawn literal (S2) and executed (X1).
...
//   S5  a drawnArgs function (Contracts' drawCard) draws that parameter through FT.l10nAuto.
//   X3  the Contracts type name, executed: ContractsApp.lua's own getTypeName (with CONTRACT_TYPE_NAMES
//       and safeGet), cut from the real source, in German: a known type reads the file's text, an
//       unknown type its raw name, no type the file's word for Contract.
//
// Usage:  node tools/test/l10n-animals-contracts-fleet-check.mjs        Exit: 0 clean, 1 any failure.
...
const KEYS = [
  "ft_animals_help_pen_cards_title", "ft_animals_help_pen_cards_body", "ft_animals_help_food_title",
  "ft_animals_help_food_body", "ft_animals_help_water_title", "ft_animals_help_water_body",
  "ft_animals_help_straw_title", "ft_animals_help_straw_body", "ft_animals_help_productivity_title",
  "ft_animals_help_productivity_body", "ft_count_pen", "ft_count_pens", "ft_animals_no_pens",
  "ft_animals_purchase_pen", "ft_auto_food", "ft_auto_water", "ft_auto_straw_clean", "ft_common_empty_lower",
  "ft_auto_plowing", "ft_auto_cultivating", "ft_auto_sowing", "ft_auto_mowing", "ft_auto_weeding",
  "ft_auto_deadwood", "ft_auto_tree_planting", "ft_auto_transport", "ft_auto_contract",
  "ft_contracts_minutes_left", "ft_contracts_hours_left", "ft_contracts_hours_minutes_left",
  "ft_auto_contracts_2", "ft_auto_shows_all_field_contracts_your_farm_has_accepted",
  "ft_auto_and_is_currently_working_on", "ft_auto_completion_updates_automatically",
  "ft_auto_time_remaining", "ft_auto_time_shown_is_in_game_time_not_real_world_time",
  "ft_auto_contracts_in_amber_are_expiring_soon_2_game_hours",
  "ft_auto_tap_t_to_close_the_tablet_and_get_back_to_work", "ft_auto_done_collect_reward",
  "ft_auto_contracts_marked_done_are_complete_but_unpaid",
  "ft_auto_visit_the_npc_on_the_map_to_dismiss_and_collect", "ft_auto_your_reward",
  "ft_auto_no_contracts_showing", "ft_auto_accept_contracts_from_npcs_on_the_map_or_via_the",
  "ft_auto_contracts_board_in_the_pause_menu_only_accepted",
  "ft_auto_contracts_appear_here_available_ones_do_not", "ft_auto_none_active", "ft_contracts_n_active",
  "ft_contracts_n_expiring", "ft_contracts_n_to_collect", "ft_auto_no_active_contracts",
  "ft_auto_accept_contracts_from_npcs_on_the_map", "ft_contracts_badge_active",
  "ft_contracts_badge_expiring", "ft_contracts_badge_collect", "ft_auto_unknown_field",
  "ft_auto_visit_npc_on_map_to_collect_reward", "ft_contracts_game_time", "ft_auto_expired",
  "ft_auto_expiring_3", "ft_auto_active_3", "ft_auto_completed_collect_reward_from_npc", "ft_auto_vehicle_list",
  "ft_auto_shows_every_motorized_vehicle_your_farm_owns",
  "ft_auto_sorted_by_fuel_level_emptiest_machines_appear_first",
  "ft_auto_so_you_can_spot_what_needs_refuelling_at_a_glance", "ft_auto_fuel_bar",
  "ft_auto_current_fuel_as_a_percentage_of_tank_capacity",
  "ft_auto_litres_remaining_and_total_capacity_shown_beside_the_ba", "ft_auto_green_50_yellow_20_red_20",
  "ft_auto_wear_bar", "ft_auto_component_wear_percentage_0_new_100_worn_out",
  "ft_auto_high_wear_reduces_efficiency_repair_before_reaching_80", "ft_auto_green_30_yellow_65_red_65",
  "ft_auto_hours_status", "ft_auto_operating_hours_total_engine_time_in_whole_hours",
  "ft_auto_ai_badge_appears_when_the_vehicle_is_driven_by_a_hired",
  "ft_auto_no_badge_parked_or_player_controlled", "ft_fleet_one_vehicle", "ft_fleet_vehicles_fmt",
  "ft_fleet_no_vehicles", "ft_fleet_no_vehicles_hint", "ft_ai_active_short", "ft_fleet_fuel",
  "ft_fleet_wear", "ft_fleet_hours_fmt",
];
...
const OTHER_PR = {
  "ft_ui_app_animals": "the app name, the home screen's key (#177, merged; l10n-home-check.mjs)",
  "ft_ui_app_contracts": "the app name, the home screen's key (#177, merged; l10n-home-check.mjs)",
  "ft_ui_app_fleet_manager": "the app name, the home screen's key (#177, merged; l10n-home-check.mjs)",
  "ft_auto_done": "the DONE status, the home screen's key (l10n-home-check.mjs)",
  "ft_auto_active_contracts": "the Dashboard's contracts card title (l10n-dashboard-check.mjs)",
  "ft_auto_unknown": "the Field Jobs PR's key (l10n-fieldjobs-check.mjs)",
  "ft_auto_fertilizing": "a Field Jobs type name (l10n-fieldjobs-check.mjs)",
  "ft_auto_spraying": "a Field Jobs type name (l10n-fieldjobs-check.mjs)",
  "ft_auto_harvesting": "a Field Jobs type name (l10n-fieldjobs-check.mjs)",
  "ft_auto_stone_picking": "a Field Jobs type name (l10n-fieldjobs-check.mjs)",
  "ft_auto_baling": "a Field Jobs type name (l10n-fieldjobs-check.mjs)",
};
...
const SOURCES = [
  {"file": "src/apps/AnimalHusbandryApp.lua", "keyFns": ["l10n", "l10nFormat"], "literals": true},
  {"file": "src/apps/ContractsApp.lua", "keyFns": ["l10n", "l10nFormat"], "literals": true, "drawnTables": ["CONTRACT_TYPE_NAMES"], "drawnArgs": {"drawCard": 2}},
  {"file": "src/apps/FleetManagerApp.lua", "keyFns": ["l10n", "l10nFormat", "ftFleetText"], "literals": true},
];
...
// ---- Draw-site rows, over the real source.
{
  ...
  for (const src of SOURCES) {
    ...
    (function walk(n) {
      ...
      if (n.type === "CallExpression") {
        ...
        // FT.l10nAuto (Contracts' drawCard status label), called directly or through pcall.
        if (src.drawnArgs) {
  ...
  // ---- X3: the Contracts type name, executed. ContractsApp.lua's own CONTRACT_TYPE_NAMES, safeGet and
  // getTypeName, cut from the real source, run in a fresh fengari state beside the real Constants.lua with
  // g_i18n answering from the real German file: a known type reads the file's text, an unknown type its
  // raw name, a mission with no type the file's word for Contract.
  let x3 = "";
  {
    const rel = "src/apps/ContractsApp.lua";
    const { text, ast } = PARSED.get(rel);
    const cut = (name) => {
      const s = ast.body.find((n) => (n.type === "FunctionDeclaration" && n.isLocal && n.identifier && n.identifier.name === name)
        || (n.type === "LocalStatement" && n.variables.length === 1 && n.variables[0].name === name));
      return s ? text.slice(s.range[0], s.range[1]) : null;
    };
    const parts = ["CONTRACT_TYPE_NAMES", "safeGet", "getTypeName"].map(cut);
    const de = ALL["de"].inside;
    const want = ["ft_auto_plowing", "ft_auto_tree_planting", "ft_auto_contract"].map((k) => (de.has(k) ? unesc(de.get(k)[0]) : null));
    if (parts.includes(null)) failures.push(`X3 ${rel}: CONTRACT_TYPE_NAMES, safeGet or getTypeName is not a top-level local; the bar cannot run the type name`);
    else if (want.includes(null)) failures.push(`X3: translation_de.xml lacks ft_auto_plowing, ft_auto_tree_planting or ft_auto_contract`);
    else {
      const L3 = lauxlib.luaL_newstate();
      lualib.luaL_openlibs(L3);
      const run3 = (buf, name) => {
        if (lauxlib.luaL_loadbuffer(L3, buf, null, to_luastring(name)) !== lua.LUA_OK || lua.lua_pcall(L3, 0, 0, 0) !== lua.LUA_OK) {
          const e = lua.lua_tojsstring(L3, -1); lua.lua_pop(L3, 1); return e;
        }
        return null;
      };
      let err = run3(readFileSync(join(ROOT, "src", "core", "Constants.lua")), "@Constants.lua");
      if (!err) {
        lua.lua_createtable(L3, 0, de.size);
        for (const [k, v] of de) { lua.lua_pushstring(L3, to_luastring(unesc(v[0]))); lua.lua_setfield(L3, -2, to_luastring(k)); }
        lua.lua_setglobal(L3, to_luastring("X3_DE"));
        err = run3(Buffer.from(`
          g_i18n = { texts = X3_DE }
          function g_i18n:hasText(k) return self.texts[k] ~= nil end
          function g_i18n:getText(k) return self.texts[k] end
          ${parts.join("\n")}
          local function m(t) return { getMissionTypeName = function() return t end } end
          X3_OUT = table.concat({ getTypeName(m("PlowMission")), getTypeName(m("TreePlantMission")),
            getTypeName(m("FooMission")), getTypeName({}) }, "|")
        `, "latin1"), "@" + rel + " (X3 cut)");
      }
      if (err) failures.push(`X3: getTypeName did not run: ${err}`);
      else {
        lua.lua_getglobal(L3, to_luastring("X3_OUT"));
        x3 = lua.lua_tojsstring(L3, -1);
        lua.lua_pop(L3, 1);
        const exp = [want[0], want[1], "FooMission", want[2]].join("|");
        if (x3 !== exp) failures.push(`X3 getTypeName in German: got ${JSON.stringify(x3)}, want ${JSON.stringify(exp)} (a known type, another, an unknown type, no type)`);
      }
    }
  }
  console.log(`  draw sites checked: ${keySites} key calls, ${litSites} drawn literals in ${SOURCES.map((s) => s.file).join(", ")}; X1: ${x1Calls} lookups executed against the files; X3: the Contracts type name executed in German`);
}
...
if (failures.length > 0) {
  ...
  console.log(`l10n-animals-contracts-fleet: ${failures.length} failure(s) over ${checked} entries (${KEYS.length} keys x ${locales.length} locales)`);
  process.exit(1);
}
console.log(`l10n-animals-contracts-fleet: PASS - ${checked} entries checked (${KEYS.length} keys x ${locales.length} locales), ${Object.keys(ALLOW).length} allowed identical pairs, ${Object.keys(ALLOW_ALL).length} own names; script rule: ${scriptChecked} values in ${locales.filter((l) => SCRIPT[l]).join(", ")}`);
