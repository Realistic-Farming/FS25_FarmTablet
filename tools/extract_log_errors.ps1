<#
.SYNOPSIS
    Extracts FarmTablet-related errors and warnings from FS25 game log.

.DESCRIPTION
    Parses the Farming Simulator 25 log.txt file and extracts all entries
    related to the FarmTablet mod, categorizing them by severity and type.
...
Write-Header "FarmTablet Log Analyzer"
...
# Patterns to match
$patterns = @{
    # FarmTablet specific patterns
    FarmTabletLine = 'FarmTablet|FS25_FarmTablet'
    ModLoaded = 'v(\d+\.\d+\.\d+)'
...
foreach ($i in 0..($lines.Count - 1)) {
    ...
    # Check if line is FarmTablet related
    if ($line -match $patterns.FarmTabletLine) {
        $stats.FarmTabletLines++
    ...
    if ($line -match $patterns.XmlParse -and $line -match 'FarmTablet') {
        $stats.XmlErrors += @{
            Line = $i + 1
            Text = $line.Trim()
            Type = "XmlError"
...
$report = @()
$report += "=" * 60
$report += "FarmTablet Error Report"
...
$report += "FarmTablet-Related Lines: $($stats.FarmTabletLines)"
